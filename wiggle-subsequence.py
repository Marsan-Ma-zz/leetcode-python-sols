...
class Solution(object):
    def wiggleMaxLength(self, nums):
        ...
        # [Ideas]
        # 1. one-pass traverse, find bounce-points count.
        #    => no reason to skip bounce-point if exists
        ...
        vecs = [b-a for a,b in zip(nums, nums[1:]) if b-a]
        if sum(vecs) == 0: return 1
        return sum(c*d < 0 for c,d in zip(vecs, vecs[1:])) + 2
        
        
        # if len(nums) < 2: return len(nums)
        
        # vecs = [b-a for a,b in zip(nums, nums[1:])]
        # if sum(vecs) == 0: return 1
        # last = vecs[0]
        # sol = 0
        # for v in vecs[1:]:
